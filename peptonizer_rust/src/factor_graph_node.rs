...
pub enum Node {
    PeptideNode { id: String, initial_belief_0: f32, initial_belief_1: f32 },
    FactorNode { id: String, parent_number: i32 },
    TaxonNode { id: String }
}
    ...
    pub fn parse_node(node: &Element) -> Result<Self, String> {
        ...
        match current_node_data.get("d2").map(String::as_str) {
            Some("factor") => {
                ...
                Ok(Node::FactorNode { id, parent_number })
            }
            Some("peptide") => {
                let initial_belief_0: f32 = current_node_data.get("d0").unwrap().parse().unwrap();
                let initial_belief_1: f32 = current_node_data.get("d1").unwrap().parse().unwrap();
                Ok(Node::PeptideNode { id, initial_belief_0, initial_belief_1 })
            }
            Some("taxon") => {
                Ok(Node::TaxonNode { id })
            }
            _ => {
